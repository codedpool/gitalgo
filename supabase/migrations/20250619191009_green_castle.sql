/*
  ...
  1. New Tables
    ...
    - `issue_labels` - Many-to-many for issue labels
  ...
  full_name text GENERATED ALWAYS AS (
    CASE 
      WHEN organization_id IS NOT NULL THEN 
        (SELECT name FROM organizations WHERE id = organization_id) || '/' || name
      ELSE 
        (SELECT username FROM profiles WHERE id = owner_id) || '/' || name
    END
  ) STORED,
  description text,
...
CREATE POLICY "Organization members are viewable by everyone" ON organization_members
  FOR SELECT USING (true);
...
CREATE POLICY "Repository owners can update their repositories" ON repositories
...
CREATE POLICY "Repository owners can delete their repositories" ON repositories
  FOR DELETE USING (owner_id = auth.uid());

-- Repository collaborators policies
CREATE POLICY "Repository collaborators are viewable by repository members" ON repository_collaborators
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Repository owners can manage collaborators" ON repository_collaborators
  FOR ALL USING (
    EXISTS (
...
-- Issues policies
CREATE POLICY "Issues are viewable by repository members" ON issues
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM repositories r
      WHERE r.id = repository_id 
...
CREATE POLICY "Authenticated users can create issues in accessible repositories" ON issues
...
CREATE POLICY "Issue authors and repository members can update issues" ON issues
  FOR UPDATE USING (
...
-- Pull requests policies (similar to issues)
CREATE POLICY "Pull requests are viewable by repository members" ON pull_requests
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM repositories r
      WHERE r.id = repository_id 
...
CREATE POLICY "Authenticated users can create pull requests in accessible repositories" ON pull_requests
  FOR INSERT WITH CHECK (
    auth.role() = 'authenticated' AND
    EXISTS (
      SELECT 1 FROM repositories r
...
-- Comments policies
CREATE POLICY "Comments are viewable by repository members" ON comments
  FOR SELECT USING (
...
CREATE POLICY "Authenticated users can star repositories" ON stars
  FOR INSERT WITH CHECK (auth.role() = 'authenticated' AND user_id = auth.uid());

CREATE POLICY "Users can unstar repositories" ON stars
